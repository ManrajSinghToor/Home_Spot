...
export default function Profile() {
  ...
  useEffect(() => {
    ...
    const history = JSON.parse(localStorage.getItem('bookingHistory')) || MOCK_BOOKINGS;
    setBookings(history);
  }, [user, navigate]);
              ...
              <div className="glass-panel" style={{ padding: '30px', border: '1px solid rgba(255,255,255,0.08)' }}>
                ...
                <div className="timeline" style={{ textAlign: 'left' }}>
                  {bookings.map((booking) => (
                    <div key={booking.id} className={`timeline-item ${booking.status === 'approved' ? 'active' : 'pending'}`}>
                      <div className="timeline-node"></div>
                      <div style={{ paddingLeft: '15px' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '10px' }}>
                          <h4 style={{ color: '#fff', fontSize: '1.1rem', fontWeight: '600', margin: 0 }}>{booking.title}</h4>
                          <span style={{
                            fontSize: '0.75rem',
                            padding: '4px 10px',
                            borderRadius: '20px',
                        ...
                        <p style={{ color: '#a1a1aa', fontSize: '0.85rem', margin: '5px 0' }}>
                          {booking.city} &bull; Booked on {booking.date}
                        </p>
